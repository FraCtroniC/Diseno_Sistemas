  ...
  if (loading) {
    return (
      <section className="space-y-6">
        <div className="rounded-[1.75rem] border border-white/70 bg-white/80 p-8 text-center text-slate-500 shadow-sm dark:text-slate-400">
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)]">
        <Card className="overflow-hidden border-unefa/15 bg-white/90 p-0 shadow-[0_24px_70px_-50px_rgba(11,87,164,0.45)] dark:bg-slate-800/90 dark:shadow-[0_24px_70px_-50px_rgba(0,0,0,0.5)]">
          <div className="border-b border-slate-100 px-6 py-5 sm:px-8 dark:border-slate-700/50">
            <p className="text-xs font-semibold uppercase tracking-[0.28em] text-unefa">Últimas publicaciones</p>
            <div className="mt-2 flex flex-wrap items-end justify-between gap-3">
              <div>
                <h3 className="text-2xl font-black text-slate-900 dark:text-slate-100">Explora los trabajos recientes</h3>
                <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">Selecciona una tarjeta para ver la ficha técnica completa.</p>
              </div>
              <p className="text-sm font-medium text-slate-500 dark:text-slate-400">{latestPublications.length} resultados</p>
            </div>
          </div>
                ...
                return (
                  <Link
                    ...
                    className={`min-w-[260px] snap-start rounded-[1.4rem] border p-5 text-left transition focus:outline-none focus:ring-2 focus:ring-unefa/40 lg:min-w-0 ${
                      isSelected
? 'border-unefa bg-[linear-gradient(180deg,rgba(11,87,164,0.08),rgba(255,255,255,1))] shadow-[0_18px_50px_-34px_rgba(11,87,164,0.55)] dark:bg-[linear-gradient(180deg,rgba(11,87,164,0.2),rgba(30,41,59,1))]'
: 'border-slate-200 bg-white hover:border-unefa/25 hover:bg-unefa/5 dark:border-slate-700 dark:bg-slate-800 dark:hover:border-unefa/25 dark:hover:bg-unefa/10'
                    }`}
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-500 dark:text-slate-400">{document.year}</p>
                        <h4 className="mt-2 text-base font-black leading-6 text-slate-900 dark:text-slate-100">{document.title}</h4>
                      </div>
                      <span className={`rounded-full px-3 py-1 text-xs font-semibold ${isSelected ? 'bg-unefa text-white' : 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300'}`}>
                        Ficha
                      </span>
                    </div>
                    ...
                    <p className="mt-4 text-xs font-medium text-slate-500 dark:text-slate-400">{document.authors.join(' · ')}</p>
      ...
      {isDetailOpen && selectedPublication ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/55 px-4 py-6 backdrop-blur-sm">
          ...
          <Card className="relative z-10 w-full max-w-4xl overflow-hidden border-unefa/15 bg-[linear-gradient(180deg,rgba(255,255,255,0.99),rgba(244,248,255,0.98))] p-0 shadow-[0_30px_90px_-35px_rgba(11,87,164,0.55)] dark:bg-[linear-gradient(180deg,rgba(30,41,59,0.99),rgba(15,23,42,0.98))] dark:shadow-[0_30px_90px_-35px_rgba(0,0,0,0.6)]">
            <div className="flex items-start justify-between gap-4 border-b border-slate-100 px-6 py-5 sm:px-8 dark:border-slate-700/50">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.28em] text-unefa">Detalle del Trabajo Académico</p>
                <h3 className="mt-2 text-2xl font-black text-slate-900 dark:text-slate-100">Ficha técnica</h3>
              </div>
              <button
                type="button"
                onClick={() => setIsDetailOpen(false)}
                className="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-unefa/30 hover:text-unefa-dark dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 dark:hover:border-unefa/30"
              >
                Cerrar
              </button>
            </div>
            ...
            <div className="grid gap-6 px-6 py-6 sm:px-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(280px,0.85fr)]">
              <div className="space-y-5">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-500 dark:text-slate-400">Título</p>
                  <h4 className="mt-2 text-3xl font-black leading-tight text-slate-900 dark:text-slate-100">{selectedPublication.title}</h4>
                </div>

                <div className="rounded-[1.4rem] border border-unefa/10 bg-white px-5 py-4 shadow-sm dark:bg-slate-800/80 dark:shadow-[0_4px_12px_-6px_rgba(0,0,0,0.5)]">
                  <p className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-500 dark:text-slate-400">Resumen</p>
                  <p className="mt-3 text-sm leading-7 text-slate-700 dark:text-slate-300">{selectedPublication.abstract}</p>
                </div>
              ...
              <div className="grid gap-4 self-start sm:grid-cols-2 lg:grid-cols-1">
                <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 dark:border-slate-600 dark:bg-slate-800">
                  ...
                  <p className="mt-2 text-sm font-medium text-slate-800 dark:text-slate-200">{selectedPublication.authors.join(', ')}</p>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 dark:border-slate-600 dark:bg-slate-800">
                  <p className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-500">Año</p>
                  <p className="mt-2 text-sm font-medium text-slate-800 dark:text-slate-200">{selectedPublication.year}</p>
                </div>
              </div>
            </div>
          </Card>
      ...
      {all.length > 0 ? (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {[
            ...
            return (
              <Card key={item.tipo} className="border-unefa/15 bg-white/90 transition hover:shadow-md dark:bg-slate-800/90 dark:shadow-[0_8px_24px_-12px_rgba(0,0,0,0.5)]">
                <p className="text-sm font-semibold text-unefa">{item.label}</p>
                <p className="mt-1 text-3xl font-black text-slate-900 dark:text-slate-100">{count}</p>
